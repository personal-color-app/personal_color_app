...
- 지도 surface는 모든 ABI/기기 호환성을 위해 앱 내 WebView + 로컬 Leaflet asset + OSM 데이터 기반 HTTPS light tile로 통일한다. Kakao Maps Android native SDK는 사용하지 않는다.
- 매장 데이터는 Kakao Local REST keyword search를 사용한다. 지도 상단은 검색 입력이 아니라 클릭 불가 상태 헤더(`주변 뷰티 매장`)이며, 지도 pan/zoom 후에만 `이 지역 재검색` 버튼을 노출한다. 지도는 너무 넓은 영역을 억지로 표시하지 않도록 최소 zoom 14에서 더 이상 축소되지 않는다. 재검색은 현재 지도 중심 좌표 기준으로 Kakao Local `size=15` 페이지를 `meta.is_end`까지 따라가되, 공식 `pageable_count` 노출 한계에 맞춰 최대 45개까지 갱신한다.
- 마커/선택 매장 카드의 `지도 앱 열기`는 Google Maps 앱에서 장소/좌표 결과 표시를 우선 실행한다. Google Maps 앱이 없거나 실패하면 Google Maps HTTPS URL을 일반 browser/default handler, Chrome/Samsung Internet/Edge/Firefox 등 명시 browser package 순으로 시도하고, 모든 handler가 없으면 링크를 클립보드에 복사한 뒤 toast로 안내한다. Google Maps Directions 화면으로 직접 진입하지 않는다.
- 지도 현재 위치는 유효한 국내 좌표 last-known이 있으면 즉시 매장을 표시해 빈 로딩 화면을 피하고, 위치가 오래됐을 때만 Fused fresh location과 Android `LocationManager` 단발 fresh 요청으로 갱신한다. 사용할 수 있는 국내 좌표가 없으면 부산대 기준 fallback으로 처리한다. 오래된 국외 last-known 좌표는 국내 매장 검색에 사용하지 않는다.
...
- `MapUiState`: `data class MapUiState(stores, selected, fallbackReason, locationLabel, centerLat/Lng, viewportLat/Lng/Zoom, canRefreshVisibleRegion, loading, activeFilter, favoriteIds)`
...
| `src/screens/map.jsx` | full-bleed 지도형 배경, 상단 header, filter chips, markers, bottom sheet |
...
| Diagnosis | analyze | preview `분석 시작` | Gemini or policy result save 후 `ResultActivity` 자동 이동 | Result 내부 기본 컬러 요약/최종 컬러 분석 안내 | 중간 완료 페이지 없이 결과 화면 |
...
| Result | commerce recommendation | 의상/메이크업 tab의 기본 컬러 요약 아래 | backend proxy Naver/Gemini commerce 호출 결과 렌더 | backend off/빈 상품/오류면 commerce 섹션 미노출 | AI-first ordering, backend on/off, crash 0 |
...
| Map | filter | `전체`, `저장` chips | in-memory favorite filter | empty state | chip별 목록 |
| Map | favorite | selected store card icon | Room favorite toggle | toast 유지 | 저장 chip 반영 |
| Map | marker details | custom marker/selected label tap | Google Maps search URL 외부 Intent | browser/default handler fallback, no-handler clipboard fallback | marker tap 후 외부 지도 열림 |
| Map | map app open | selected store card `지도 앱 열기` | 좌표 우선 Google Maps search URL/geo intent | browser/default handler fallback, no-handler clipboard fallback | no crash, Directions 직접 진입 없음 |
...
| Digit 2FA | 손글씨 canvas, TFLite 판정, model-unavailable continue button, reset. 터치 입력과 bitmap 변환 좌표는 canvas bounds 안으로 clamp하며, 하단에는 불필요한 방어 설명 문장을 노출하지 않는다. | 숫자 1 인증 안정성, 빈/작은 stroke 안내, 크래시 금지 |
| Main | 6-item scroll-safe drawer, hero CTA, quick action, recent -> result, color story pills. Drawer 항목과 로그아웃은 작은 화면에서도 완전한 텍스트로 표시한다. | bottom tab 또는 동일 navigation density 정밀화 |
...
| Result | Compose 4 scroll-safe tabs, dot indicator, save icon, overflow share, map recommendation card, mypage primary | user-visible swipe pager 정밀화 |
| Map | WebView + 로컬 Leaflet + OSM 데이터 기반 light tile interactive map, 클릭 불가 상태 헤더 `주변 뷰티 매장`, `전체`/`저장` filters, 지도 조작 후 `이 지역 재검색`, 핀 끝 좌표 anchored marker, zoom 15 이하 custom cluster, 같은 장소 중복 제거, draggable bottom sheet, selected-card favorite/`지도 앱 열기`. 선택 매장은 모든 zoom에서 개별 핀 + 라벨을 유지하고, 같은 장소 중복 제거 시에도 선택 매장을 우선 보존한다. zoom-out은 최소 zoom 14에서 멈춰 너무 넓은 영역/과도한 매장 표시 요구를 만들지 않는다. zoom-out 때 선택 라벨이 bottom sheet에 걸리면 Leaflet `panBy`로 위쪽 안전 영역에 보정한다. zoom만으로 목록은 바꾸지 않는다. 목록/마커 갱신은 명시 재검색 버튼에서만 일어난다. 재검색 후에는 현재 zoom 문맥을 유지하고, 선택 매장이 결과에 있으면 목록 첫 카드로 고정하며 없으면 첫 결과를 새 선택으로 둔다. `저장` 필터에서는 상단 헤더, count, 바텀시트 제목, 리스트, 지도 마커가 모두 저장 매장 기준으로 동기화되며 제목은 `저장 매장 N곳`으로 표시한다. 하단 매장 목록의 handle은 사용자가 위/아래로 드래그해 목록 높이를 조절할 수 있고, 최소/최대 높이 안에서만 움직인다. Kakao Local 검색은 `size=15` 페이지를 `meta.is_end`까지 읽고 공식 `pageable_count` 노출 한계에 맞춰 최대 45개까지 표시한다. 로딩 중에는 빈 결과 문구(`0곳`, `찾지 못했습니다`)를 먼저 표시하지 않고 기존 목록 또는 `매장 확인 중` 상태를 유지한다. Marker tap은 내부 선택이 아니라 Google Maps 외부 이동이다. Kakao Local/seed 데이터는 검증 가능한 영업 상태를 보장하지 않으므로 지도 카드에는 영업 상태 문구를 표시하지 않는다. 지도 WebView는 정적 asset `web/oliveme_map.html`을 1회 로드하고 `window.OliveMeMapRuntime.setData(...)`로 매장 payload만 갱신한다. 동일 HTML reload를 피하고, Main/Result/MyPage에서는 기본 매장 데이터만 warmup한다. WebView/Chromium shell prewarm은 저공간 AVD에서 ANR을 만들 수 있어 사용하지 않는다. MapActivity는 lightweight skeleton과 매장 목록을 먼저 표시한 뒤 WebView attach를 지연해 focus ANR을 피한다. Leaflet animation/tile update 부담 완화, zoom button/viewport bridge debounce, 현재 viewport 근처 마커/클러스터 렌더링으로 화면 밖 DOM을 줄인다. | 모든 기기/ABI에서 pan/zoom 가능한 실제 tile map 유지, `지도 앱 열기`는 Google Maps/browser/clipboard fallback |
| MyPage | avatar, stats row, scroll-safe `리포트`/`이력`/`저장 매장` tabs, magazine report, overflow save/share, clickable history/store, empty state, redo | HTML spacing/thumbnail fidelity 정밀화 |
...
- seed 계정 로그인 시 `DemoSeedRepository.ensureDemoData`가 진단 이력과 컬러 스토리를 한 번 seed한다. 즐겨찾기 매장은 mock처럼 보이지 않도록 자동 seed하지 않고 사용자가 지도에서 저장한 매장만 유지한다. 예전 빌드가 넣어둔 `pnu-*` seed 즐겨찾기는 로그인 seed 정리 단계에서 삭제한다. 사용자 화면에는 `데모 계정` 표현을 노출하지 않는다.
...
- 화면 표시는 항상 Room에 저장된 진단 결과를 우선한다. Room 결과가 없으면 초기/빈 상태용 policy sample만 사용한다. 저장된 Room 색상이 6색 미만이거나 어두운 색으로 과도하게 몰린 구형 row이면 `DiagnosisRepository`가 현재 subtype 정책 팔레트로 표시용 결과만 보정한다. 조회 중 `recommended_colors`/`product_recommendations`를 delete/insert로 다시 쓰지 않는다.
- Result의 커머스 AI 추천은 색을 새로 판정하지 않는다. 이미 저장된 `palette`/`avoidColors`와 타입, Naver 상품 목록을 backend Gemini 요약에 보내 상품 추천 문구와 `picks`를 생성한다. `picks`는 Naver 상품 목록의 rank 중 2-3개를 골라 이유와 함께 내려주며, Android는 의상/메이크업 탭에서 `기본 컬러 요약` 바로 아래에 `AI 추천`과 `AI가 고른 상품` 썸네일 행을 먼저 표시한 뒤 `실시간 상품 추천`, `최종 컬러 분석` 순서로 보여준다. 메이크업 상품은 backend에서 립/아이/베이스/치크 검색을 합쳐 균형 있게 정렬하고, 네일/베이스코트 같은 비메이크업 잡상품은 제외한다. Gemini commerce 요약이 429/timeout/parse failure로 실패하면 backend가 상품명 나열 대신 팔레트와 카테고리 역할 중심의 짧은 로컬 추천 요약과 rank 기반 picks를 만들어 `AI 추천` 위치를 비워두지 않는다. Android UI는 title/link/image가 비어 있는 상품을 다시 제외하고, 안전하지 않은 상품 URL은 열지 않는다.
...
| Gemini commerce summary | backend `.env`의 `GEMINI_API_KEY` | Naver 상품과 진단 팔레트를 묶은 `AI 추천` 요약 | backend 로컬 추천 요약으로 대체. 현재 2026-06-21 QA에서 Gemini commerce smoke는 `429 RESOURCE_EXHAUSTED`였고, Naver 상품은 계속 표시됨 |
...
- `DigitCanvas`는 stroke path를 bitmap으로 렌더링한다. Pointer 입력과 bitmap 변환 시 stroke 좌표는 canvas bounds 내부로 clamp해 캔버스 밖 드래그가 모델 입력에 새지 않게 한다.
...
| Map | filters | `전체`, `저장` | filter state | local favorite filter |
| Map | favorite | 선택된 card에서 Room save/remove + in-memory ids | heart favorite | 실패해도 UI 유지 |
| Map | 지도 앱 열기 | 선택된 card에서 좌표 우선 map URL/geo intent | external map/browser | 앱/브라우저 없음 toast + 링크 복사 |
...
- 2FA canvas 밖으로 드래그해도 stroke와 bitmap 입력이 canvas 내부에 머무르고, 하단 불필요 문구가 없는지 확인한다.
- Main drawer open/close, drawer 전체 항목 탭. 작은 화면에서도 drawer 텍스트와 로그아웃이 잘리지 않아야 한다.
- Diagnosis camera cancel, gallery cancel, sample photo, preview, analyze, Result 자동 이동.
- Result tabs, dot indicator, save snackbar, share fallback, map/mypage 이동. 긴 탭/칩 행은 화면 폭을 밀지 않고 스크롤 또는 ellipsis로 안정 표시한다.
- Map location denied fallback, filters, store select, favorite toggle.
- Map header는 검색 입력처럼 보이지 않는 상태 헤더이고, 지도 pan/zoom 후 `이 지역 재검색` 버튼이 보인다.
- Map bottom sheet handle을 위/아래로 드래그하면 매장 목록 높이가 실제로 조절된다.
- Map `이 지역 재검색`은 Kakao Local `meta.is_end` pagination을 따라 최대 45개까지 표시한다. 단일 Kakao keyword search의 공식 노출 한계 밖 결과는 단일 요청으로 전부 표시할 수 없다.
- Map은 zoom 14 미만으로 축소되지 않아 과도하게 넓은 영역을 한 번에 표시하지 않는다.
- zoom 15 이하에서 선택 매장은 라벨을 유지하고 주변 매장은 숫자 클러스터로 묶이며, 클러스터 tap은 앱 내부 지도 bounds 확대만 수행한다.
- 지도 로딩 중에는 `근처 매장 0곳`이나 `근처 매장을 찾지 못했습니다`를 먼저 보여주지 않는다. 유효한 국내 last-known 좌표가 있으면 즉시 목록을 표시하고 fresh 위치 갱신을 병행한다.
- 재검색 후 선택 매장이 결과에 있으면 목록 첫 카드로 고정하고, 없으면 첫 결과로 선택을 맞춘다.
- zoom/pan 스트레스에서는 zoom button debounce, viewport bridge debounce, stale store request guard가 동작해 no-crash 상태를 유지한다.
- `저장` 필터는 상단 헤더, count, 바텀시트 제목, 리스트, 지도 마커가 모두 저장 매장 기준으로 동기화되고 `저장 매장 N곳`으로 표시된다.
- OSM marker tap과 selected card `지도 앱 열기`는 Google Maps 외부 Intent로 이동하고, Google Maps/브라우저가 없어도 링크 복사 toast로 no-crash 처리된다.
- MyPage tabs, report share/save, latest result open, store open, redo diagnosis. 프로필 칩과 탭 텍스트는 작은 화면에서 줄바꿈/잘림 없이 표시한다.
...
- seed 계정: 바로 시작 후 최근 리포트/이력/컬러 스토리가 비어 있지 않고 사용자 화면 이름은 `게스트`인지 확인한다. 저장 매장은 seed가 아니라 사용자가 실제 지도 결과에서 저장한 항목만 표시되는지 확인한다.
...
- 상품 탭 AI-first 순서: Result `의상`/`메이크업` 탭은 `기본 컬러 요약` compact swatch card를 먼저 보여주고, backend 상품이 있으면 `AI 추천`/`AI가 고른 상품`/`실시간 상품 추천`을 즉시 이어 보여준다. 기존 상세 팔레트와 파트별 적용 정보는 하단 `최종 컬러 분석`으로 이동한다.
- 상품 proxy on: backend `.env`에 Naver/Gemini key가 있고 emulator QA에서 `adb reverse tcp:8787 tcp:8787`가 설정되어 있으면 Result `의상`/`메이크업` 탭에 `기본 컬러 요약`, `AI 추천`, `AI가 고른 상품`, `실시간 상품 추천`, 실제 상품 `ImageView ... 썸네일`, 하단 `최종 컬러 분석`이 표시되는지 확인한다.
- 메이크업 상품 정확도: backend가 `메이크업` 요청에서 립/아이/베이스/치크 후보를 각각 섞고, `추천 아이템` 같은 generic copy와 네일/베이스코트 잡상품이 없는지 반복 request와 Android screenshot/UI tree로 확인한다.
- 상품 proxy off/fault: `127.0.0.1:8787` 연결 실패, adb reverse 없음, HTTP 500, timeout, 빈 상품, malformed 상품이면 Result `의상`/`메이크업` 탭에서 `AI 추천`과 `실시간 상품 추천` 섹션이 조용히 미노출되고 `기본 컬러 요약`/`최종 컬러 분석`과 `logcat -b crash` 0 lines를 확인한다.
...
- OSM WebView 지도, Google Maps 외부 Intent, 위치 권한, 즐겨찾기/지도 앱 열기 상태.
