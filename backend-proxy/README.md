...
- Backend proxy: `/health`, Gemini proxy smoke, Naver Shopping Search, FCM test endpoint 준비.
...
- `POST /v1/products/recommendations`: 진단 타입, 팔레트, 검색 키워드로 Naver Shopping 상품을 가져온 뒤 Gemini가 `AI 추천` 요약을 생성합니다. Android는 이 응답으로 AI 요약을 먼저 보여주고, Naver 상품 썸네일 카드를 이어서 렌더링합니다.
...
2026-06-02 QA에서 `.env` 설정 후 `/health`는 `geminiConfigured=true`, `naverShoppingConfigured=true`를 반환했고, `/v1/products/search?query=lip&display=3`는 `source=naver-shopping`으로 실제 Naver 상품 3개를 반환했습니다. `/v1/products/recommendations`는 `AI 추천` 요약과 image URL이 있는 Naver 상품을 반환했습니다. 샘플 얼굴 4장 proxy Gemini 분석도 모두 HTTP 200을 반환했습니다.
